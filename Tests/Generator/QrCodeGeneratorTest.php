...
class QrCodeGeneratorTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->testImagePath  =  __DIR__.'/../../Test/Files/qrcode';
        $this->getInstance();
    }

    public function getInstance($fileInCache=false)
    {
        $this->qrCodeGenerator = new QrCodeGenerator($this->getFileHandlerMock($fileInCache));
    }

    public function testCreateBarcode()
    {
        $this->assertFalse(file_exists($this->testImagePath));
        $this->assertEquals($this->testImagePath, $this->qrCodeGenerator->get('ABC123'));
    ...
    public function testCreateBarcodeInCache()
    {
        ...
        $this->assertEquals('oldFileFromCache', $this->qrCodeGenerator->get('ABC123'));
    }
